...
def run_enigma_benchmark(model):
    ...
    results = {}
    if not prompts: return results
    for i, prompt in enumerate(prompts):
        print(f"  Processing ENIGMA prompt {i+1}/{len(prompts)}: {prompt[:70]}...")
        ...
        try:
            ...
            error_message = str(e)
            print(f"DEBUG: Caught Exception Type in ENIGMA: {type(e)}")
            print(f"DEBUG run_enigma_benchmark: RAW ERROR MSG: {error_message}")
            ...
            print(f"DEBUG run_enigma_benchmark: is_quota_error_flag_local = {is_quota_error_flag_local}")
            if is_quota_error_flag_local:
                api_calls_failed_quota += 1
                quota_error_hit = True
                print(f"DEBUG run_enigma_benchmark: quota_error_hit SET TO TRUE")
            else:
                api_calls_failed_other += 1
        if SECONDS_BETWEEN_API_CALLS > 0: time.sleep(SECONDS_BETWEEN_API_CALLS)
        print(f"DEBUG run_enigma_benchmark: Just before break check: quota_error_hit = {quota_error_hit}")
        if quota_error_hit:
            print("INFO: API Quota Exceeded in ENIGMA benchmark. Skipping remaining prompts.")
            break
    return results
...
def run_visual_benchmark(model):
    ...
    results = {}
    if not prompts_data: return results
    for i, data in enumerate(prompts_data):
        ...
        try:
            ...
            error_message = str(e)
            print(f"DEBUG: Caught Exception Type in VISUAL: {type(e)}")
            print(f"DEBUG run_visual_benchmark: RAW ERROR MSG: {error_message}")
            results[prompt_key] = f"ERROR: {error_message}"
            is_quota_error_flag_local = False
            if "429" in error_message and ("quota" in error_message.lower() or "rate limit" in error_message.lower() or "resource has been exhausted" in error_message.lower()):
                is_quota_error_flag_local = True
            print(f"DEBUG run_visual_benchmark: is_quota_error_flag_local = {is_quota_error_flag_local}")
            if is_quota_error_flag_local:
                api_calls_failed_quota += 1
                quota_error_hit = True
                print(f"DEBUG run_visual_benchmark: quota_error_hit SET TO TRUE")
            else:
                api_calls_failed_other += 1
        if SECONDS_BETWEEN_API_CALLS > 0: time.sleep(SECONDS_BETWEEN_API_CALLS)
        print(f"DEBUG run_visual_benchmark: Just before break check: quota_error_hit = {quota_error_hit}")
        if quota_error_hit:
            print("INFO: API Quota Exceeded in VISUAL benchmark. Skipping remaining prompts.")
            break
    return results
...
def run_lipogram_benchmark(model):
    ...
    for i, prompt in enumerate(prompts):
        ...
        try:
            ...
            error_message = str(e)
            print(f"DEBUG: Caught Exception Type in LIPOGRAM: {type(e)}")
            print(f"DEBUG run_lipogram_benchmark: RAW ERROR MSG: {error_message}")
            results[prompt] = f"ERROR: {error_message}"
            is_quota_error_flag_local = False
            if "429" in error_message and ("quota" in error_message.lower() or "rate limit" in error_message.lower() or "resource has been exhausted" in error_message.lower()):
                is_quota_error_flag_local = True
            print(f"DEBUG run_lipogram_benchmark: is_quota_error_flag_local = {is_quota_error_flag_local}")
            if is_quota_error_flag_local:
                api_calls_failed_quota += 1
                quota_error_hit = True
                print(f"DEBUG run_lipogram_benchmark: quota_error_hit SET TO TRUE")
            else:
                api_calls_failed_other += 1
        if SECONDS_BETWEEN_API_CALLS > 0: time.sleep(SECONDS_BETWEEN_API_CALLS)
        print(f"DEBUG run_lipogram_benchmark: Just before break check: quota_error_hit = {quota_error_hit}")
        if quota_error_hit:
            print("INFO: API Quota Exceeded in LIPOGRAM benchmark. Skipping remaining prompts.")
            break
    return results
...
def run_relogio_benchmark():
    ...
    for i, prompt in enumerate(prompts):
        ...
        results[prompt] = {
            "status": "CONCEPTUAL: Image generation not implemented...",
            "notes": "This script does not perform image generation...",
            "intended_example_output_path": saved_image_path
        }
    print("  --> CLOCK benchmark is conceptual and does not generate images...")
    return results
